   ...
   <div class="detail_query">
   		...
   		<div style="width: 100%;">
			...
			<div class="div_add">
				...
				${aroundSuggestStoreVo.viewReplyIllegalNumber}
			</div>
		</div>
		<div class="detail_row">
			...
			<div class="div_add" style="width:60%">
				...
				${aroundSuggestStoreVo.startNumber}
			</div>
		</div>
    ...
    <table id="dg" class="easyui-datagrid"
    		url="aroundSuggestStore/findCommentAll?aroundSuggestStoreId=${id}"
	...
	<script type="text/javascript">
	   ...
	   function searchComment(){
        ...
        function deleteComment() {
            ...
            if (row){
                $.messager.confirm('提示','确定要删除吗?',function(r){
                    if (r){
                        $.post('aroundSuggestStore/deleteComment',{id:row.id,delFlag:'1'},function(result){
                            if (result.type=='Success'){
                            	$.messager.show({    // show error message
                                    title: 'Success',
                                    msg: result.Msg
                                });
                            	parent.$('#addAroundSuggestStoreFormUI').attr("src","aroundSuggestStore/toCommentAroundSuggestStore/${id}");
                            } else {
                                $.messager.show({    // show error message
                                    title: 'Error',
                                    msg: result.Msg
        ...
        function toCommentReply() {
        	var aroundSuggestStoreId = $('#id').val();
        	var row = $('#dg').datagrid('getSelected');
            if (row){
            	parent.$('#addAroundSuggestStoreFormUI').attr("src","aroundSuggestStore/toReplyComment/"+row.id+"/"+aroundSuggestStoreId);
            	parent.$('#dlg').dialog('open').dialog('setTitle','本网格推荐周边店回复的回复管理');
            }else{
