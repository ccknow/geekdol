...
<body>
   ...
   <div style="height: 70px; width: 100%;">
        <form id="ff" method="post">
            ...
            <div style="float: left; padding: 20px 0 0 20px;">
                <label for="status">小区名称:</label> 
                <input  name="villageName" class="easyui-textbox" style="width: 100px;">
            </div>
    ...
    <table id="dg"  class="easyui-datagrid" style="height:620px;"
            url="intimateNews/findzhaunfaList" 
        ...
        <thead>
            <tr>
                ...
                <th field="publishTime" width="45"  formatter="conversion">发布时间</th>
                <th field="replyFlag" width="20" formatter="shifouhuifuorzhiding">允许回复</th>
                <th field="hotnumber" width="35" >浏览/回复/违规</th>
            </tr>
        </thead>
    </table>
    
    <div id="toolbar">
    <shiro:hasPermission name="IntimateNewF:manager">
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-zhuanfa" plain="true" onclick="editUser()">转发</a>
    </shiro:hasPermission>
    </div>
        ...
        <div id="dlg" class="easyui-dialog" style="padding:10px 20px;width:800px;height:540px;overflow:hidden;"
         ...
         <div id="dlg-buttons">
            <!-- <a href="javascript:void(0)" class="easyui-linkbutton c6" iconCls="icon-ok" onclick="saveUser('0')" style="width:90px">保存</a> -->
            <a id="saveBtn" href="javascript:void(0)" class="easyui-linkbutton c6" iconCls="icon-storenotice" onclick="saveUser('1')" style="width:90px">发布</a>
    ...
    function newUser(){
    	$('#saveBtn').show();
        $('#addUserFormUI').attr("src","intimateNews/zhuanfaform");
        $('#dlg').dialog('open').dialog('setTitle','转发');
        $('#fm').form('clear');
    }
    ...
    function shifouhuifuorzhiding(val,row){
       ...
       function searchProvince(){
        ...
        function editUser(){
            ...
            var row = $('#dg').datagrid('getSelected');
            if (row){
                $('#addUserFormUI').attr("src","intimateNews/tozhuanfaform/"+row.id);
                $('#dlg').dialog('open').dialog('setTitle','编辑');
            }else{
                $.messager.alert('提示信息','请先选择要转发的记录。','info');
            }
        }
    ...
    function onClickRow(index){
        ...
        <shiro:hasPermission name="IntimateNewF:manager">
        $('#toolbar').append("<a href='javascript:void(0)' class='easyui-linkbutton' iconCls='icon-zhuanfa' plain='true' onclick='editUser()'>转发</a>");
        </shiro:hasPermission>
