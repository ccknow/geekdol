...
<body>
   ...
   <div style="width: 90%;">
   		<input type="hidden" id="inCommentId" name="inCommentId" value="${inCommentId}" />
   		<div  style="margin-top: 10px" >
			<label>所属小区:</label> 
			${intimateNewsVo.villageName}
			&nbsp;&nbsp;&nbsp;
			<label>(贴心报)浏览/评论/违规:</label> 
			${intimateNewsVo.hotnumber}
			&nbsp;&nbsp;&nbsp;
			<label>(贴心报)点赞/倒赞:</label> 
			${intimateNewsVo.likesOrhate}
		</div>
		
		<div   style="margin-top: 10px">
            <label>评论内容:</label> 
            ${intimateNewsCommentVo.content}
        </div>
        
        <div   style="margin-top: 10px">
            <label>评论人:</label> 
            ${intimateNewsCommentVo.insName}
            &nbsp;&nbsp;&nbsp;
            <label>评论时间:</label> 
            ${commentDatetime}
           <label>浏览/评论/违规:</label> 
           <span id="hotnumber">${intimateNewsCommentVo.viewReplyIllegalNumber}</span>
            <label>点赞/倒赞:</label> 
            ${intimateNewsCommentVo.likesOrhate}
        </div>
        <div style="margin-top: 10px" >
		<form id="ff" method="post">
				...
				<input type="button" onclick="searchCommentReply()" value="查询" />
		</form>
		</div>
    </div>
    <div style="margin-top:30px">
    <table id="dg" class="easyui-datagrid" style="height: 620px;"
    		url="intimateNews/findCommentReplyAll?inCommentId=${intimateNewsCommentVo.id}"
        ...
        function deleteCommentReply() {
            ...
            if (row){
                $.messager.confirm('提示','确定要删除吗?',function(r){
                    if (r){
                        $.post('intimateNews/deleteCommentReply',{id:row.id,delFlag:'1'},function(result){
