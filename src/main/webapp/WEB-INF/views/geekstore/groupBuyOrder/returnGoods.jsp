	...
	<div class="ftitle"></div>
        <form id="fm" method="post" novalidate style="height: 100px; width: 100%;">
        	<div class="fitem" >
				...
				<select id="detailResult" name="detailResult" class="easyui-combobox" style="width: 120px;" data-options="editable:false">
					<option value="05">${TY}</option>
					<option value="06">${JJ}</option>
				</select>
			</div>
			<div class="fitem">
                <label>退货地址:</label>
                <input name="returnAdress" class="easyui-textbox" required="true" style="width: 180px;">
            </div>
            <div class="fitem">
                <label>理由:</label>
                <textarea  name="detailResultReason" class="easyui-validatebox" required="true" style="height:50px; width: 500px"></textarea>
            </div>        
        </form>
        <form id="fm1234" method="post" novalidate style="height: 100px; width: 100%;">
        	<div class="fitem">
            <table width="700" height="80" border="0">
					  <tr>
					    <td height="30">退款理由</td>
					  </tr>
					  <tr>
					    <td><textarea  name="orderReturnReason" class="easyui-validatebox" readonly="readonly" style="height:50px; width: 500px"></textarea></td>
					  </tr>
					  	<c:forEach items="${attachmentList}" var="item">
					  		<img src="${item.url}" style="width:50px;float:left">
					  	</c:forEach>
					</table> 
            </div>        	       
        </form>
        <form id="fm123" method="post" novalidate style="height: 100px; width: 100%;">
            ...
            <input name="returnId" id="returnId" type="hidden" value="${returnId}">         
        </form>
</body>
...
$('#fm123').form('load','groupBuyOrder/detailsInit/'+id+"/"+serialCode);
$('#fm1234').form('load','groupBuyOrder/returnDetailsInit/'+id+"/"+returnId);
var url = "";
function imgUrl(value){
	alert("123");
	value='<img style="width:30px; height:30px" src="/geekdol/CooperationStore/231857af-3a53-4d34-bc48-a646648fbfd5.jpg">';
	return value;
}
function optSubmit(){
	var url= "groupBuyOrder/returnGoodSave/"+${id};
    $('#fm').form('submit',{
        url: url,
        onSubmit: function(){
