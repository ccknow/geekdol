...
<body>
 	<div style="height: 45px; width: 100%; padding-top: 12px;">
		...
		<div class="fitem" style="width: 58%;float: right;">
			<label>总评论/好评/中评/差评:</label>
			${grouponGoods.countStar }/${grouponGoods.goodStar }
			/${grouponGoods.commonStar }/${grouponGoods.badStar }
		</div>
	</div> 
   <div style="height: 70px; width: 100%;"> 
		<form id="ff" method="post">
			...
			<input type="hidden" name="goodsId" id="goodsId" value="${grouponGoods.id }">
		</form>
	</div> 
     <table id="dg" class="easyui-datagrid" style="height: 250px;"
     		url="groupongoods/findGrouponCommentList/${grouponGoods.id }" 
        ...
        <thead>
            <tr>
                ...
                <th field="statusStr" width="13%">违规状态</th>
            </tr>
        </thead>
    </table>
    <div id="toolbar">
   ...
   <script type="text/javascript">
	   ...
	   function searchGrouponComment(){
		   $('#ff').form('submit',{  
			    url: "groupongoods/findGrouponCommentList/"+id,
			    onSubmit: function(){  
        ...
        function deleteGrouponComment(){
            ...
            if (row){
                $.messager.confirm('提示','确定要删除吗?',function(r){
                    if (r){
                        $.post('groupongoods/deleteGrouponComment',{id:row.id},function(result){
        ...
        function grouponCommentReturninfo(){
        	...
        	var row = $('#dg').datagrid('getSelected');
            if (row){
            	$('#addUserFormUI').attr("src","groupongoods/toViewCommentReturninfo/"+goodsId+"/"+row.id);
