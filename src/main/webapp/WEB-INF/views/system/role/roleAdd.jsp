                       ...
                       <tbody>
                         ...
                         <c:forEach items="${moduleList}" var="module" varStatus="status">
                             <tr>
                                 ...
                                 <c:choose>
                                 <c:when test="${zname != fn:substringBefore(module.name,'】') }">
                                 ${fn:substringBefore(module.name,'】')}】<input type="checkbox"  value="${status.index}" onclick="changeCheckbox(this,${number})">
                                 </c:when>
                                 <c:otherwise>
                                 <c:set var="number" value="${status.index}+1"></c:set>
                                 </c:otherwise>
                                 </c:choose>
                                 </td>
                                 ...
                                 <c:forEach  items="${module.permissions}" var="permissions">
                                     <c:if test="${permissions.value eq 'view' }">
                                         <input type="checkbox"  name="checkbox" value="${permissions.perId}">
                                     </c:if>
                                 </c:forEach>
                                 </td>   
                                 <td style="text-align: center;border-left:1px solid #8DB2E3;border-top:1px solid #8DB2E3">
...
function optSubmit(){
 	var checkNumber=$("input[name='checkbox'][checked]").length;
	if(checkNumber==0){
		 $.messager.show({
             title: '提示',
             msg: "请选择权限！"
...
function changeCheckbox(obj,number){
