   ...
   <div style="height: 70px; width: 100%;">
		<form id="ff" method="post">
			...
			<div style="float: left; padding: 20px 0 0 20px;">
				<label for="provinceName">省名称:</label> 
				<input class="easyui-textbox" type="text" name="provinceName" />
			</div>
			<div style="float: left; padding: 20px 0 0 20px;" >
    ...
    <table id="dg"  class="easyui-datagrid" style="height: 620px;"
        ...
        <thead>
            <tr>
                <th field="provinceName" width="50">省名称</th>
                <th field="provinceCode" width="50" >省编码</th>
    ...
    <div id="toolbar">
    	<shiro:hasPermission name="Province:manager">
    ...
    <script type="text/javascript">
	   ...
	   function searchProvince(){
        ...
        function newUser(){
        	$('#addUserFormUI').attr("src","province/add");
        ...
        function editUser(){
            ...
            if (row){
            	if(row.status=='0'){
            		$('#addUserFormUI').attr("src","province/edit/"+row.provinceId);
        ...
        function destroyUser(){
            ...
            if (row){
                $.messager.confirm('提示','确定要删除吗?',function(r){
                    if (r){
                        $.post('province/delete',{provinceId:row.provinceId},function(result){
            ...
            if (row){
            	if(row.status=='0'){
            		 $.messager.confirm('提示','确定要禁用吗?',function(r){
                         if (r){
                             $.post('province/jystatus',{provinceId:row.provinceId,status:row.status},function(result){
                                 if (result.type=="Success"){
                                	 $.messager.show({    // show error message
                                         title: 'Success',
                                         msg:result.msg,
            ...
            if (row){
            	if(row.status=='1'){           		 
            		 $.messager.confirm('提示','确定要启用吗?',function(r){
                         if (r){
                             $.post('province/qystatus',{provinceId:row.provinceId,status:row.status},function(result){
                                 if (result.type=="Success"){
                                	 $.messager.show({    // show error message
                                         title: 'Success',
                                         msg:result.msg,
